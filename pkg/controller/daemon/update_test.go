...
func TestDaemonSetSurgingUpdatesPods(t *testing.T) {
	...
	ds.Spec.Template.Spec.Containers[0].Image = "foo2/bar2"
	ds.Spec.UpdateStrategy.Type = apps.SurgingRollingUpdateDaemonSetStrategyType
	intStr := intstr.FromInt(maxSurge)
	ds.Spec.UpdateStrategy.SurgingRollingUpdate = &apps.SurgingRollingUpdateDaemonSet{MaxSurge: &intStr}
...
func TestPruneSurgingDaemonPods(t *testing.T) {
	...
	newVersionLabels := map[string]string{"name": "simple-daemon", "type": "production", apps.DefaultDaemonSetUniqueLabelKey: newVersionHash}
	...
	for _, c := range []struct {
		...
		ds: func() *apps.DaemonSet {
			ds := newDaemonSet("x")
			ds.Spec.UpdateStrategy.Type = apps.OnDeleteDaemonSetStrategyType
			return ds
		}(),
		pods: []*v1.Pod{pod0, pod1},
		want: []*v1.Pod{pod0, pod1},
...
func TestGetSurgeNumbers(t *testing.T) {
	for _, c := range []struct {
		...
		name: "Two nodes with pods, with no new hash",
		manager: func() *daemonSetsController {
			manager, _, _, _ := newTestController()
			addNodes(manager.nodeStore, 0, 2, nil)
			return manager
		...
		name: "Two nodes with pods, with one new hash",
		manager: func() *daemonSetsController {
			manager, _, _, _ := newTestController()
			addNodes(manager.nodeStore, 0, 2, nil)
			return manager
		...
		nodeToPods: func() map[string][]*v1.Pod {
			mapping := make(map[string][]*v1.Pod)
			pod0a := newPod("pod-0a", "node-0", simpleDaemonSetLabel, nil)
			pod0b := newPod("pod-0b", "node-0", simpleDaemonSetLabelWithHash, nil)
			pod1 := newPod("pod-1", "node-1", simpleDaemonSetLabel, nil)
			mapping["node-0"] = []*v1.Pod{pod0a, pod0b}
			mapping["node-1"] = []*v1.Pod{pod1}
			return mapping
		...
		nodeToPods: func() map[string][]*v1.Pod {
			mapping := make(map[string][]*v1.Pod)
			pod0 := newPod("pod-0", "node-0", simpleDaemonSetLabel, nil)
			pod1 := newPod("pod-1", "node-0", simpleDaemonSetLabelWithHash, nil)
		...
		c.manager.dsStore.Add(c.ds)
		maxSurge, numSurge, err := c.manager.getSurgeNumbers(c.ds, c.nodeToPods, "new-hash")
		if err != nil {
