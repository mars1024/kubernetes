...
func (rt mockRuntimeService) UpdateContainerResources(id string, resources *runtimeapi.LinuxContainerResources, specAnnotations map[string]string) error {
	return rt.err
}
...
func TestCPUManagerGenerate(t *testing.T) {
	...
	for _, testCase := range testCases {
		t.Run(testCase.description, func(t *testing.T) {
			...
			mgr, err := NewManager(nil, "", nil, testCase.cpuPolicyName, 5*time.Second, machineInfo, testCase.nodeAllocatableReservation, sDir)
