...
// NOTE: original file :pkg/kubelet/cm/cpumanager/cpu_assignment.go
package allocators
...
// TODO(yuzhi.wx) supports spread cpu allocation mode
func takeByTopology(topo *topology.CPUTopology, availableCPUs cpuset.CPUSet, numCPUs int) (cpuset.CPUSet, error) {
	...
	for _, s := range acc.freeSockets() {
		if acc.needs(acc.topo.CPUsPerSocket()) {
			glog.V(4).Infof("[CPUAllocator] takeByTopology: claiming socket [%d]", s)
	...
	for _, c := range acc.freeCores() {
		if acc.needs(acc.topo.CPUsPerCore()) {
			glog.V(4).Infof("[CPUAllocator] takeByTopology: claiming core [%d]", c)
	...
	for _, c := range acc.freeCPUs() {
		glog.V(4).Infof("[CPUAllocator] takeByTopology: claiming CPU [%d]", c)
		if acc.needs(1) {
