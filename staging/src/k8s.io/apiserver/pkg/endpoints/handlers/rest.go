	...
	return func(w http.ResponseWriter, req *http.Request) {
		...
		if admit != nil && admit.Handles(admission.Connect) {
			...
			if mutatingAdmission, ok := admit.(admission.MutationInterface); ok {
				err = mutatingAdmission.Admit(admission.NewAttributesRecordWithContext(opts, nil, scope.Kind, namespace, name, scope.Resource, scope.Subresource, admission.Connect, false, userInfo, req.Context()))
				if err != nil {
					scope.err(err, w, req)
					return
				}
			...
			if validatingAdmission, ok := admit.(admission.ValidationInterface); ok {
				err = validatingAdmission.Validate(admission.NewAttributesRecordWithContext(opts, nil, scope.Kind, namespace, name, scope.Resource, scope.Subresource, admission.Connect, false, userInfo, req.Context()))
				if err != nil {
					scope.err(err, w, req)
					return
				}
