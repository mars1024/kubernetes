...
func (s *tokenReviewLister) ShallowCopyWithTenant(tenant multitenancy.TenantInfo) interface{} {
	return &tokenReviewLister{
		indexer: s.indexer,
		tenant:  tenant,
	}
}
...
func (s *tokenReviewLister) Get(name string) (*v1beta1.TokenReview, error) {
	if s.tenant == nil {
		...
		debug.PrintStack()
		return nil, fmt.Errorf("cannot get tokenreview w/o specifying tenant")
	}
