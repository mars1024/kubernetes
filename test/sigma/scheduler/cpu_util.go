...
type resourceItem struct {
	caseIndex       int
	pod             *v1.Pod
	containerResult *swarm.ContainerResult
...
func parseFromPod(tc *testContext, resourceItem *resourceItem, p *v1.Pod) (error, *allocatedResult) {
	...
	if resourceItem.cpushare {
		if newNode, err := tc.cs.CoreV1().Nodes().Get(p.Spec.NodeName, metav1.GetOptions{}); err == nil {
...
func parseFromContainerResult(resourceItem *resourceItem, p *sigma_v3.Pod) (error, *allocatedResult) {
	result := &allocatedResult{
		index:       resourceItem.caseIndex,
		sn:          p.PodSn,
		cpushare:    resourceItem.cpushare,
		cpuset:      p.PodStatus.ContainerStatuses[0].AllocResource.CpuSet,
		cpusetShare: p.PodStatus.ContainerStatuses[0].AllocResource.CpuSetShare,
		appName:     p.Labels[sigma_v3.AppName],
		duName:      p.Labels[sigma_v3.DeployUnit],
	}

	return nil, result
}

func parseFromAntContainerResult(nodeName string, resourceItem *resourceItem, allocPlan *antsigma.AllocPlan) (error, *allocatedResult) {
	result := &allocatedResult{
		index:       resourceItem.caseIndex,
		sn:          allocPlan.InstanceSn,
		cpushare:    resourceItem.cpushare,
		cpuset:      allocPlan.CpuSet,
		cpusetShare: allocPlan.CpuSet, // TODO(kubo.cph), in ant cpushare mode, allocPlan.CpuSet should be empty, which means binding all CPU cores.
		appName:     allocPlan.AppName,
		duName:      allocPlan.DeployUnit,
	}

	// In ant cpushare mode, allocPlan.CpuSet should be empty, which means binding all CPU cores.
	if len(allocPlan.CpuSet) == 0 {
		// TODO(kubo.cph), set all cores here to fit checkSharePool.
		node := swarm.GetNode(nodeName)
		cpuNum := node.LocalInfo.CpuNum
		for i := 0; i < cpuNum; i++ {
			result.cpusetShare = append(result.cpusetShare, i)
		}
	}
...
func testPreviewSigmaCase(test resourceCase) int {
	name := "scheduler" + "-" + string(uuid.NewUUID())
	config := &swarm.ContainerOption{}
	config = &swarm.ContainerOption{
		Resource: swarm.Resource{
			CPUCount: int(test.cpu / 1000),
			Memory:   test.mem,
			DiskSize: test.ethstorage,
		},
		Name: name,
		Labels: map[string]string{
			"ali.RequestId":     name,
	...
	if test.spreadStrategy != "" {
		config.Labels["ali.CpuTopologyStrategy"] = test.spreadStrategy
	}
	...
	if test.cpushare {
		...
		config.Labels["ali.CpuSetMode"] = "share"
	}

	if test.affinityConfig != nil {
		for k, v := range test.affinityConfig {
			config.Labels[k] = strings.Join(v, ",")
		}
	}
...
func previewAndVerifySigmaCase(tc *testContext, caseIndex int, test resourceCase) {
	...
	if test.shouldScheduled {
		if len(tc.testCases) > 0 {
			Expect(option != nil && option.Count == tc.testCases[caseIndex].previewCount).Should(BeTrue(),
				"CaseName:%s,requirementId:%s, Case[%d], sigma, expect pod to be scheduled successfully. params: %+v, expect:%d,actually:%d", tc.caseName, name, caseIndex, tc.testCases[caseIndex], tc.testCases[caseIndex].previewCount, option.Count)
		} else {
			Expect(option != nil && option.Count == test.previewCount).Should(BeTrue(),
				"CaseName:%s,requirementId:%s, Case[%d], sigma, expect pod to be scheduled successfully. params: %+v, option: %+v", tc.caseName, name, caseIndex, nil, option)
		}
	} else {
		if len(tc.testCases) > 0 {
			Expect(option != nil && option.Count == tc.testCases[caseIndex].previewCount).Should(BeFalse(),
				fmt.Sprintf("CaeName:%s,requirementId:%s, Case[%d], sigma, expect pod failed to be scheduled.params:%+v, expect:%d,actually:%d", tc.caseName, name, caseIndex, tc.testCases[caseIndex], tc.testCases[caseIndex].previewCount, option.Count))
		} else {
			Expect(option != nil && option.Count == test.previewCount).Should(BeFalse(),
				fmt.Sprintf("CaeName:%s,requirementId:%s, Case[%d], sigma, expect pod failed to be scheduled.params:%+v", tc.caseName, name, caseIndex, nil))
		}
	}
}

func createAndVerifySigmaCase(tc *testContext, caseIndex int, test resourceCase) {
	name := "scheduler-" + strconv.Itoa(caseIndex) + "-" + string(uuid.NewUUID())
	config := &swarm.ContainerOption{}
	if env.Tester == env.TesterJituan {
		config = &swarm.ContainerOption{
			Resource: swarm.Resource{
				CPUCount: int(test.cpu / 1000),
				Memory:   test.mem,
				// 这里需要做处理，传过来的disksize 经过 2.0 apiserver 处理以后，最后一位被当做单位
				// 所以乘以 10 用于占位
				DiskSize: test.ethstorage * 10,
			},
			Name: name,
			Labels: map[string]string{
				"ali.RequestId":     name,
			...
			ImageName: "reg.docker.alibaba-inc.com/ali/os:7u2",
		}
	} else {
		config = &swarm.ContainerOption{
			Resource: swarm.Resource{
	...
	if env.Tester == env.TesterJituan {
		if tc.isSigmaNotMock == false {
			config.Labels["ali.Scheduler.TestMode"] = "true"
		}
	}
	...
	if option != nil && option.ID != "" {
		...
		tc.resourceToDelete[caseIndex] = &resourceItem{caseIndex: caseIndex, containerResult: containerResult, success: true, cpushare: test.cpushare}
	}

	if test.shouldScheduled {
		if len(tc.testCases) > 0 {
			Expect(option != nil && option.ID != "").Should(BeTrue(),
				"CaseName:%s,requirementId:%s, Case[%d], sigma, expect pod to be scheduled successfully. params: %+v", tc.caseName, name, caseIndex, tc.testCases[caseIndex])
		} else {
			Expect(option != nil && option.ID != "").Should(BeTrue(),
				"CaseName:%s,requirementId:%s, Case[%d], sigma, expect pod to be scheduled successfully. params: %+v, option: %+v", tc.caseName, name, caseIndex, nil, option)
		}
		...
		if len(tc.testCases) > 0 {
			Expect(option != nil && option.ID != "").Should(BeFalse(),
				fmt.Sprintf("CaeName:%s,requirementId:%s, Case[%d], sigma, expect pod failed to be scheduled.params:%+v", tc.caseName, name, caseIndex, tc.testCases[caseIndex]))
		} else {
			Expect(option != nil && option.ID != "").Should(BeFalse(),
				fmt.Sprintf("CaeName:%s,requirementId:%s, Case[%d], sigma, expect pod failed to be scheduled.params:%+v", tc.caseName, name, caseIndex, nil))
		}
...
func createAndVerifySigmaUpdateCase(tc *testContext, caseIndex int, test resourceCase) {

	containerName := tc.resourceToDelete[test.updateIndex].containerResult.ContainerName
	if env.Tester == env.TesterJituan {
		containerName = tc.resourceToDelete[test.updateIndex].containerResult.ContainerSN
	}
	Expect(containerName).NotTo(BeEmpty(), "containername of sigma2.0 should not be empty")

	config := &swarm.ContainerUpdateOption{
		Resource: swarm.Resource{
			Memory: test.mem,
		}}

	err := swarm.UpdateContainer(containerName, config)
	if tc.resourceToDelete == nil {
		tc.resourceToDelete = map[int]*resourceItem{}
	}
	...
	if test.shouldScheduled {
		if len(tc.testCases) > 0 {
			Expect(err).NotTo(HaveOccurred(),
				"CaseName:%s, Case[%d], sigma, expect pod to be scheduled successfully. params:%+v", tc.caseName, caseIndex, tc.testCases[caseIndex])
		} else {
			Expect(err).NotTo(HaveOccurred(),
				"CaseName:%s, Case[%d], sigma, expect pod to be scheduled successfully. params:%+v, option:%+v", tc.caseName, caseIndex, nil, config)
		}
	} else {
		if len(tc.testCases) > 0 {
			Expect(err).Should(HaveOccurred(),
				fmt.Sprintf("CaeName:%s, Case[%d], sigma, expect pod failed to be scheduled.params:%+v", tc.caseName, caseIndex, tc.testCases[caseIndex]))
		} else {
			Expect(err).Should(HaveOccurred(),
				fmt.Sprintf("CaeName:%s, Case[%d], sigma, expect pod failed to be scheduled.params:%+v", tc.caseName, caseIndex, nil))
		}
	}
}

func createAndVerifySigmaCasePreview(tc *testContext, caseIndex int, test resourceCase) {
	Name := "scheduler-" + string(uuid.NewUUID())
	...
	preview, err := swarm.PreviewCreateContainerWithOption(config)
	Expect(err).NotTo(HaveOccurred())

	if test.shouldScheduled {
		framework.Logf("expect preview create >= 1")
		Expect(preview).Should(BeNumerically(">=", 1),
			fmt.Sprintf("sn :%s expect preview create to be >=1, get %d", Name, preview))
		framework.Logf("sn:%s, Case[%d], expect pod to be scheduled successfully.", Name, caseIndex)
	} else {
		framework.Logf("sn:%s, expect preview create == 0", Name)
		Expect(preview).Should(BeNumerically("==", 0),
			fmt.Sprintf("sn:%s, expect preview create to == 0, get %d", Name, preview))
		framework.Logf("sn:%s, Case[%d], expect pod failed to be scheduled.", Name, caseIndex)
	}
}
...
func createAndVerifyK8sPod(tc *testContext, caseIndex int, test resourceCase) {
	...
	if tc.globalRule != nil {
		if tc.globalRule.CpuSetMutex.AppConstraints != nil {
			isCpuMutex := false
			for _, appName := range tc.globalRule.CpuSetMutex.AppConstraints {
										...
										MatchExpressions: []metav1.LabelSelectorRequirement{
											{
												...
												Values:   tc.globalRule.CpuSetMutex.AppConstraints,
											},
										},
									},
								},
	...
	if test.shouldScheduled {
		err := framework.WaitTimeoutForPodRunningInNamespace(tc.cs, pod.Name, pod.Namespace, waitForPodRunningTimeout)
		Expect(err).To(BeNil())

		newPod, err := tc.cs.CoreV1().Pods(pod.Namespace).Get(pod.Name, metav1.GetOptions{})
		...
		err := framework.WaitForPodNameUnschedulableInNamespace(tc.cs, pod.Name, pod.Namespace)
		Expect(err).To(BeNil())
...
func releaseResource(tc *testContext, caseIndexes []int) map[string]map[string]struct{} {
	...
	for caseIndex, p := range tc.resourceToDelete {
		...
		if p.containerResult != nil {
			sigmaRes := p.containerResult
			glog.Infof("sigma clean name: %s", sigmaRes.ContainerSN)
			swarm.DeleteContainer(sigmaRes.ContainerSN)
			if env.Tester == env.TesterJituan {
				swarm.EtcdDelete(fmt.Sprintf("/pod/%s/%s/%s", sigmaRes.Site, sigmaRes.DeployUnit, sigmaRes.ContainerSN))
			}
			containerSnMap := hostSnToContainerSnSet[sigmaRes.HostSN]
			if containerSnMap == nil {
				containerSnMap = map[string]struct{}{}
			}
			containerSnMap[sigmaRes.ContainerSN] = struct{}{}
			hostSnToContainerSnSet[sigmaRes.HostSN] = containerSnMap
		}
		p.beDeleted = true
	}
...
func waitContainerDestroyed(tc *testContext, containerSnMap map[string]struct{}, hostSn string) error {
	if env.Tester == env.TesterJituan {
		return wait.Poll(2*time.Second, 5*time.Minute, func() (done bool, err error) {
			podMap := swarm.GetHostPod(hostSn)
			if len(podMap) == 0 {
				return true, nil
			}
		...
		// For ant sigma 2.0, we just sleep 5 seconds to wait the scheduler to flush the memory.
		...
		return wait.Poll(2*time.Second, 5*time.Minute, func() (done bool, err error) {
			...
			for containerSn := range containerSnMap {
				planKey := fmt.Sprintf("/nodes/allocplans/%s/%s/%s", site, nsInfo[0].ServiceTag, containerSn)
				data, _ := swarm.EtcdGet(planKey)
				if len(data) > 0 {
					return false, nil
				}
				pod, err := tc.cs.CoreV1().Pods(tc.f.Namespace.Name).Get(containerSn, metav1.GetOptions{})
				if err == nil {
					By(fmt.Sprintf("Waiting for %s:%+v to be destroyed.", containerSn, pod))
		...
		case requestTypeSigma:
			if test.cpushare && env.Tester == env.TesterAnt {
				if tc.CPUOverQuotaRatio == 0 {
					tc.CPUOverQuotaRatio = 1
				}
				extLabels := map[string]string{
					"CpuSetMode":        "share",
					"CpuShareOverQuota": strconv.FormatFloat(tc.CPUOverQuotaRatio, 'E', -1, 64),
				}
...
func (tc *testContext) isCpuMono(result *allocatedResult) bool {
	...
	if tc.globalRule.CpuSetMonopolize.AppConstraints != nil {
		for _, appName := range tc.globalRule.CpuSetMonopolize.AppConstraints {
	...
	if tc.globalRule.CpuSetMonopolize.DUConstraints != nil {
		for _, duName := range tc.globalRule.CpuSetMonopolize.DUConstraints {
...
func (tc *testContext) isCpuMutex(result *allocatedResult) bool {
	...
	if tc.globalRule.CpuSetMutex.DUConstraints != nil {
		for _, duName := range tc.globalRule.CpuSetMutex.DUConstraints {
			if result.duName == duName {
				return true
			}
		}
...
func checkResult(tc *testContext, checkFuncList []checkFunc) {
	...
	for _, ri := range tc.resourceToDelete {
		if ri.success && !ri.beDeleted {
			if ri.pod != nil {
				if newPod, err := tc.cs.CoreV1().Pods(ri.pod.Namespace).Get(ri.pod.Name, metav1.GetOptions{}); err == nil {
			...
			containerResult := ri.containerResult
			if containerResult != nil {
				switch env.Tester {
				case env.TesterJituan:
					podKey := fmt.Sprintf("/pod/%s/%s/%s", containerResult.Site, containerResult.DeployUnit, containerResult.ContainerSN)
					if podBytes, err := swarm.EtcdGet(podKey); podBytes != nil {
						v3Pod := &sigma_v3.Pod{}
						if err := json.Unmarshal(podBytes, v3Pod); err == nil {
							if err, cpuSetResult := parseFromContainerResult(ri, v3Pod); cpuSetResult != nil {
								allocatedResults = append(allocatedResults, cpuSetResult)
							} else {
								Expect(err).To(BeNil(), "caseName: %s, podBytes: %v, got: %s", tc.caseName, string(podBytes), err)
							}
						} else {
							Expect(err).To(BeNil(), "caseName: %s, podBytes: %v, got: %s", tc.caseName, string(podBytes), err)
						}
					} else {
						Expect(err).To(BeNil(), "caseName: %s, expect err not be nil, got: %s", tc.caseName, err)
					}
				case env.TesterAnt:
					allocPlanKey := fmt.Sprintf("/nodes/allocplans/%s/%s/%s", containerResult.Site, containerResult.HostSN, containerResult.ContainerSN)
					if allocPlanBytes, err := swarm.EtcdGet(allocPlanKey); allocPlanBytes != nil {
						allocPlan := &antsigma.AllocPlan{}
						if err := json.Unmarshal(allocPlanBytes, allocPlan); err == nil {
							if err, cpuSetResult := parseFromAntContainerResult(tc.nodeName, ri, allocPlan); cpuSetResult != nil {
								allocatedResults = append(allocatedResults, cpuSetResult)
							} else {
								Expect(err).To(BeNil(), "caseName: %s, allocPlanBytes: %v, got: %s", tc.caseName, string(allocPlanBytes), err)
							}
						} else {
							Expect(err).To(BeNil(), "caseName: %s, allocPlanBytes: %v, got: %s", tc.caseName, string(allocPlanBytes), err)
						}
				...
				default:
					Expect("err").To(BeNil(), "caseName: %s, env.Tester: %s not supported now", tc.caseName, env.Tester)
				}
			}
		}
	}
	...
	for _, checkFun := range checkFuncList {
		...
		if ret == false && env.Tester == env.TesterJituan && tc.nodeName != "" {
			podMap := swarm.GetHostPod(tc.nodeName)
...
func checkContainerShareCPUShouldBeNil(tc *testContext, cpuSetResults []*allocatedResult, cpuIDInfoMap map[int]api.CPUInfo) (bool, string) {
	for _, result := range cpuSetResults {
		...
		case requestTypeSigma:
			if env.Tester == env.TesterAnt && (len(result.cpusetShare) != len(tc.localInfo.CPUInfos)) {
				return false, fmt.Sprintf("failed to check ant sigma checkContainerShareCPUShouldBeNil caseName: %s, caseIndex: %v, sn: %s, cpuset: %v, should be all",
					tc.caseName, result.index, result.sn, result.cpuset)
			}

			if env.Tester == env.TesterJituan && !(len(result.cpusetShare) > 0) {
				return false, fmt.Sprintf("failed to check jituan sigma checkContainerShareCPUShouldBeNil caseName: %s, caseIndex: %v, sn: %s, cpuset: %v, should not be empty",
					tc.caseName, result.index, result.sn, result.cpuset)
			}
		}
	}
...
// 校验node的 sharePool == localinfo.cpuset - allocated cpusets
func checkNodeSharePool(tc *testContext, cpuSetResults []*allocatedResult, cpuIDInfoMap map[int]api.CPUInfo) (bool, string) {
	if tc.CPUOverQuotaRatio == 0 {
		tc.CPUOverQuotaRatio = 1.0
	}
	...
	sort.Ints(sharePool)
	cpusetShare := make([]int, 0, len(cpuIDInfoMap))
	if newNode, err := tc.cs.CoreV1().Nodes().Get(tc.nodeName, metav1.GetOptions{}); err == nil {
		sharePoolStr := newNode.Annotations[api.AnnotationNodeCPUSharePool]
		if sharePoolStr != "" {
			cpusharePool := &api.CPUSharePool{}
			if err := json.Unmarshal([]byte(sharePoolStr), cpusharePool); err != nil {
				return false, fmt.Sprintf("unmarshal %s data %+v err", api.AnnotationNodeCPUSharePool, sharePoolStr)
			}
			if len(cpusharePool.CPUIDs) > 0 {
				for _, cpuID := range cpusharePool.CPUIDs {
					cpusetShare = append(cpusetShare, int(cpuID))
				}
				sort.Ints(cpusetShare)
			}
		}
	}

	if !reflect.DeepEqual(cpusetShare, sharePool) {
		return false, fmt.Sprintf("failed to check checkNodeSharePool caseName: %s, caseIndex: %v,  cpusetShare: %v, not equal expect sharePool: %+v",
			tc.caseName, cpuSetResults[len(cpuSetResults)-1].index, cpusetShare, sharePool)
	}
	return true, ""
}
...
func checkContainerCpuMutexCPUID(tc *testContext, cpuSetResults []*allocatedResult, cpuIDInfoMap map[int]api.CPUInfo) (bool, string) {
	if tc.globalRule == nil || len(tc.globalRule.CpuSetMutex.AppConstraints) == 0 {
		return true, ""
	}
