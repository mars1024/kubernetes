...
var _ = Describe("[sigma-3.1][sigma-scheduler][cpushare][cpu]", func() {
	...
	// 1.25倍 超卖场景下的SameCoreFirst，验证分配的物理核优先，k8s的cpuset和cpushare同时进行
	// 步骤 要求每个容器分配的cpu个数不能低于2个，否则这个case会验证失败
	// 1.  1/4 整机核 cpushare sigma（预期成功）
	// 2.  1/4 整机核 cpuset k8s（预期成功）
	// 3.  1/4 整机核 cpushare k8s（预期成功）
	// 4.  1/4 整机核 cpuset sigma（预期成功）
	// 5.  1/4 整机核 cpuset k8s（预期成功）
	// 6.  1/4 整机核 cpuset sigma（预期失败）
	...
	It("[ali] overQuotaCPUShareMix001", func() {
		...
		testContext := &testContext{
			caseName:          "overQuotaCPUShareMix001",
			cs:                cs,
			localInfo:         localInfo,
			f:                 f,
			testCases:         tests,
