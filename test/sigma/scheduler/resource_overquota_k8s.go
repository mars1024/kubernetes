...
var _ = Describe("[sigma-3.1][sigma-scheduler][resource][Serial]", func() {
	...
	It("[p1] ResourceOverquotaK8s001 cpu-over-quota=2 & mem-over-quota=1.1.", func() {
		...
		tests := []struct {
			...
			{
				cpu:                       *resource.NewMilliQuantity(pod2CPU-1000, resource.DecimalSI),
				mem:                       *resource.NewQuantity(pod2Memory-100*1024*1024, resource.DecimalSI),
				ethstorage:                *resource.NewQuantity(pod2Disk-100*1024*1024, resource.DecimalSI),
			...
			{
				cpu:                       *resource.NewMilliQuantity(pod1CPU, resource.DecimalSI),
				mem:                       *resource.NewQuantity(100*1024*1024, resource.DecimalSI),
				ethstorage:                *resource.NewQuantity(100*1024*1024, resource.DecimalSI),
				expectedAvailableResource: []int64{allocatableCPU - pod1CPU - pod2CPU + 1000, allocatableMemory - pod1Memory - pod2Memory + 100*1024*1024, allocatableDisk - pod1Disk - pod2Disk + 100*1024*1024},
				spreadStrategy:            sigmak8sapi.SpreadStrategySameCoreFirst,
