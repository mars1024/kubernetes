...
type LogicInfo struct {
	...
	PhyHostIdentity       string                       `json:",omitempty"` //表示ECS宿主机(物理机)的全局唯一表示
	...
	FpgaOverQuota         float64                      `json:",omitempty"` // "FpgaOverQuota": 3.5,
	TianjiClusterName     string                       `json:",omitempty"` // "TianjiClusterName": AY49A, //天基集群名字
	...
	CpuReserveQuota       int                          `json:",omitempty"` // 预留cpu
	MemoryReserveQuota    int64                        `json:",omitempty"` // 预留mem
	DiskReserveQuota      int64                        `json:",omitempty"` // 预留disk
	PhyIpRange            string                       `json:",omitempty"` // 物理机网断
	EniQuota              int64                        `json:",omitempty"` // 弹性网卡的配额
	IsEcs                 bool                         `json:",omitempty"`
	EcsIpRange            string                       `json:",omitempty"`
	PubNetIpRange         string                       `json:",omitempty"`
	EcsInstanceId         string                       `json:",omitempty"`
	EcsDeploymentSetId    string                       `json:",omitempty"`
	EcsRegionId           string                       `json:",omitempty"`
}
