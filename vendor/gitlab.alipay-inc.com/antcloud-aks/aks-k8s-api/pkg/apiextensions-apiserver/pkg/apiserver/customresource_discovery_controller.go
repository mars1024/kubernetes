...
type DiscoveryController struct {
	...
	syncFn func(tgv util.TenantGroupVersion) error

	queue workqueue.RateLimitingInterface
}

func (c *DiscoveryController) ShallowCopyWithTenant(tenant multitenancy.TenantInfo) interface{} {
	copied := *c
	copied.crdLister = copied.crdLister.(multitenancymeta.TenantWise).ShallowCopyWithTenant(tenant).(listers.CustomResourceDefinitionLister)
	return &copied
}
...
func (c *DiscoveryController) sync(tgv util.TenantGroupVersion) error {
	version := tgv.GroupVersion
	tenant := tgv.TenantHash.TenantInfo()
	c = c.ShallowCopyWithTenant(tenant).(*DiscoveryController)
	...
	if !foundGroup {
		c.groupHandler.unsetDiscovery(tgv.TenantGroup())
		c.versionHandler.unsetDiscovery(tgv)
		return nil
	}
	...
	c.groupHandler.setDiscovery(tgv.TenantGroup(), discovery.NewAPIGroupHandler(Codecs, apiGroup))

	if !foundVersion {
		c.versionHandler.unsetDiscovery(tgv)
		return nil
	}
	c.versionHandler.setDiscovery(tgv, discovery.NewAPIVersionHandler(Codecs, version, discovery.APIResourceListerFunc(func() []metav1.APIResource {
...
func (c *DiscoveryController) processNextWorkItem() bool {
	...
	err := c.syncFn(key.(util.TenantGroupVersion))
	if err == nil {
		c.queue.Forget(key)
		return true
	}
...
func (c *DiscoveryController) enqueue(obj *apiextensions.CustomResourceDefinition) {
	for _, v := range obj.Spec.Versions {
		tenant, err := multitenancyutil.TransformTenantInfoFromAnnotations(obj.Annotations)
		if err != nil {
			utilruntime.HandleError(fmt.Errorf("crd discovery controller hitting malformed key: %v", err))
		}
		c.queue.Add(
			util.TenantGroupVersion{
				multitenancyutil.GetHashFromTenant(tenant),
				schema.GroupVersion{Group: obj.Spec.Group, Version: v.Name}},
		)
	}
}
