...
import (
	...
	"gitlab.alipay-inc.com/antcloud-aks/aks-k8s-api/pkg/apiextensions-apiserver/pkg/controller/establish"
	"gitlab.alipay-inc.com/antcloud-aks/aks-k8s-api/pkg/apiextensions-apiserver/pkg/controller/finalizer"
)

func (c *crdHandler) ShallowCopyWithTenant(tenant multitenancy.TenantInfo) interface{} {
	copied := *c
...
func (r *crdHandler) ServeHTTP(w http.ResponseWriter, req *http.Request) {
	...
	if err != nil {
		responsewriters.InternalError(w, req, fmt.Errorf("no valid tenant found in the context"))
		return
	}

	r = r.ShallowCopyWithTenant(tenant).(*crdHandler)
...
func (r *crdHandler) updateCustomResourceDefinition(oldObj, newObj interface{}) {
	...
	if !apiextensions.IsCRDConditionTrue(newCRD, apiextensions.Established) &&
		...
		if r.masterCount > 1 {
			r.establishingController.QueueCRD(util.CRDKeyFunc(newCRD), 5*time.Second)
		} else {
			r.establishingController.QueueCRD(util.CRDKeyFunc(newCRD), 0)
		}
	}
