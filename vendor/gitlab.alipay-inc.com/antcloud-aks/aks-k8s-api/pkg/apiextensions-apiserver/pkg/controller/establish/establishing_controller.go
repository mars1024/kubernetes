...
func (ec *EstablishingController) ShallowCopyWithTenant(tenant multitenancy.TenantInfo) interface{} {
	copied := *ec
	copied.crdClient = ec.crdClient.(multitenancymeta.TenantWise).ShallowCopyWithTenant(tenant).(client.CustomResourceDefinitionsGetter)
	copied.crdLister = ec.crdLister.(multitenancymeta.TenantWise).ShallowCopyWithTenant(tenant).(listers.CustomResourceDefinitionLister)
	return &copied
}
...
func (ec *EstablishingController) sync(key string) error {
	...
	ec = ec.ShallowCopyWithTenant(tenant).(*EstablishingController)
