...
func (c *CRDFinalizer) ShallowCopyWithTenant(tenant multitenancy.TenantInfo) interface{} {
	copied := *c
	copied.crdClient = c.crdClient.(multitenancymeta.TenantWise).ShallowCopyWithTenant(tenant).(client.CustomResourceDefinitionsGetter)
	copied.crdLister = c.crdLister.(multitenancymeta.TenantWise).ShallowCopyWithTenant(tenant).(listers.CustomResourceDefinitionLister)
	return &copied
}
...
func (c *CRDFinalizer) sync(key string) error {
	...
	c = c.ShallowCopyWithTenant(tenant).(*CRDFinalizer)
...
func (c *CRDFinalizer) deleteInstances(crd *apiextensions.CustomResourceDefinition) (apiextensions.CustomResourceDefinitionCondition, error) {
	...
	// NOTE(yue9944882): odd to manually instantiate a context, isnt it?
	user := &user.DefaultInfo{Extra: make(map[string][]string)}
	tenant, err := multitenancyutil.TransformTenantInfoFromAnnotations(crd.Annotations)
	if err != nil {
		return apiextensions.CustomResourceDefinitionCondition{}, err
	}
	if err := multitenancyutil.TransformTenantInfoToUser(tenant, user); err != nil {
		return apiextensions.CustomResourceDefinitionCondition{}, err
	}
...
func (c *CRDFinalizer) enqueue(obj *apiextensions.CustomResourceDefinition) {
	key, err := cache.MultiTenancyKeyFuncWrapper(cache.DeletionHandlingMetaNamespaceKeyFunc)(obj)
	if err != nil {
