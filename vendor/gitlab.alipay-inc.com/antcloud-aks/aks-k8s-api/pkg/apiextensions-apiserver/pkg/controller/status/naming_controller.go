...
func (c *NamingConditionController) ShallowCopyWithTenant(tenant multitenancy.TenantInfo) interface{} {
	copied := *c
...
func NewNamingConditionController(
	...
	c.crdMutationCache = multitenancycache.NewIntegerResourceVersionMutationCache(informerIndexer, informerIndexer, 60*time.Second, false)
...
func (c *NamingConditionController) getAcceptedNamesForGroup(group string) (allResources sets.String, allKinds sets.String) {
	...
	for _, curr := range list {
		...
		key, _ := multitenancycache.MultiTenancyKeyFuncWrapper(cache.MetaNamespaceKeyFunc)(curr)
...
func (c *NamingConditionController) sync(key string) error {
	...
	c = c.ShallowCopyWithTenant(tenant).(*NamingConditionController)
	if err != nil {
		return err
	}
