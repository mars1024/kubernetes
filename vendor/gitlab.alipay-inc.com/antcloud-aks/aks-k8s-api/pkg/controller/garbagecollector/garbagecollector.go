...
func (gc *GarbageCollector) attemptToDeleteWorker() bool {
	...
	if feature.DefaultFeatureGate.Enabled(multitenancy.FeatureName) {
		// HACK(zuoxiu.jm): Ignore unknown resources
		fqKind := schema.FromAPIVersionAndKind(n.identity.APIVersion, n.identity.Kind)
		_, err := gc.restMapper.RESTMapping(fqKind.GroupKind(), fqKind.Version)
		if err != nil {
			return true
		}
		gc = gc.ShallowCopyWithTenant(n.identity.Tenant).(*GarbageCollector)
	}
...
func (gc *GarbageCollector) classifyReferences(item *node, latestReferences []metav1.OwnerReference) (
	...
	for _, reference := range latestReferences {
		if feature.DefaultFeatureGate.Enabled(multitenancy.FeatureName) {
			...
			_, err := gc.restMapper.RESTMapping(fqKind.GroupKind(), fqKind.Version)
			if err != nil {
				// HACK(zouxiu.jm): ignore unknown resources
				continue
			}
		}
...
func (gc *GarbageCollector) attemptToOrphanWorker() bool {
	...
	if feature.DefaultFeatureGate.Enabled(multitenancy.FeatureName) {
		fqKind := schema.FromAPIVersionAndKind(owner.identity.APIVersion, owner.identity.Kind)
		_, err := gc.restMapper.RESTMapping(fqKind.GroupKind(), fqKind.Version)
		if err != nil {
			return true
		}
		gc = gc.ShallowCopyWithTenant(owner.identity.Tenant).(*GarbageCollector)
	}
