...
type NoExecuteTaintManager struct {
	...
	taintedNodesLock *sync.Mutex // change to pointer to keep the lock singleton
	...
	tenantInfo multitenancy.TenantInfo
}
...
func (tc *NoExecuteTaintManager) cancelWorkWithEvent(nsName types.NamespacedName) {
	if tc.tenantInfo != nil {
		key := KeyFromNamespaceAndTenantInfo(nsName, tc.tenantInfo)
		if tc.taintEvictionQueue.CancelWork(key) {
...
func (tc *NoExecuteTaintManager) processPodOnNode(
	...
	if !allTolerated {
		...
		tc.cancelWorkWithEvent(podNamespacedName)
		if tc.tenantInfo != nil {
			tc.taintEvictionQueue.AddWork(NewWorkArgsWithTenantInfo(podNamespacedName.Name, podNamespacedName.Namespace, tc.tenantInfo), time.Now(), time.Now())
			return
		}
	...
	if tc.tenantInfo != nil {
		key := KeyFromNamespaceAndTenantInfo(podNamespacedName, tc.tenantInfo)
		scheduledEviction = tc.taintEvictionQueue.GetWorkerUnsafe(key)
	}
	...
	if tc.tenantInfo != nil {
		tc.taintEvictionQueue.AddWork(NewWorkArgsWithTenantInfo(podNamespacedName.Name, podNamespacedName.Namespace, tc.tenantInfo), startTime, triggerTime)
		return
	}
...
func (tc *NoExecuteTaintManager) handlePodUpdate(podUpdate *podUpdateItem) {
	if utilfeature.DefaultFeatureGate.Enabled(multitenancy.FeatureName) {
		...
		tc = tc.ShallowCopyWithTenant(tenantInfo).(*NoExecuteTaintManager)
	}
	...
	taints, ok := func() ([]v1.Taint, bool) {
		...
		if utilfeature.DefaultFeatureGate.Enabled(multitenancy.FeatureName) {
			key, _ := controller.KeyFunc(&v1.Node{
				ObjectMeta: metav1.ObjectMeta{
					Annotations: pod.Annotations,
					Name:        pod.Spec.NodeName,
				},
			})
			taints, ok = tc.taintedNodes[key]
		}
		return taints, ok
	}()
...
// TODO recorder here does not support multitenancy
func (tc *NoExecuteTaintManager) emitPodDeletionEvent(nsName types.NamespacedName) {
	...
	if tc.tenantInfo != nil {
		key := KeyFromNamespaceAndTenantInfo(nsName, tc.tenantInfo)
		tc.recorder.Eventf(ref, v1.EventTypeNormal, "TaintManagerEviction", "Marking for deletion Pod %s", key)
		return
	}
...
func (tc *NoExecuteTaintManager) emitCancelPodDeletionEvent(nsName types.NamespacedName) {
	...
	if tc.tenantInfo != nil {
		key := KeyFromNamespaceAndTenantInfo(nsName, tc.tenantInfo)
		tc.recorder.Eventf(ref, v1.EventTypeNormal, "TaintManagerEviction", "Cancelling deletion of Pod %s", key)
		return
	}
