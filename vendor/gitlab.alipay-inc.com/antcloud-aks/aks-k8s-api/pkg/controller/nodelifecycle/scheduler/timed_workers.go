...
func (w *WorkArgs) KeyFromWorkArgs() string {
	if w.TenantInfo != nil {
		return KeyFromNamespaceAndTenantInfo(w.NamespacedName, w.TenantInfo)
	}
	return w.NamespacedName.String()
}

func KeyFromNamespaceAndTenantInfo(nsName types.NamespacedName, tenantInfo multitenancy.TenantInfo) string {
	return fmt.Sprintf("%s%c%s%c%s%c%s", tenantInfo.GetTenantID(), types.Separator,
		tenantInfo.GetWorkspaceID(), types.Separator,
		tenantInfo.GetClusterID(), types.Separator,
		nsName.String())
}

// NewWorkArgs is a helper function to create new `WorkArgs`
func NewWorkArgs(name, namespace string) *WorkArgs {
	return &WorkArgs{NamespacedName: types.NamespacedName{Namespace: namespace, Name: name}}
}

func NewWorkArgsWithTenantInfo(name, namespace string, tenantInfo multitenancy.TenantInfo) *WorkArgs {
	return &WorkArgs{NamespacedName: types.NamespacedName{Namespace: namespace, Name: name}, TenantInfo: tenantInfo}
}
