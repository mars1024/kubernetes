...
type APIAggregator struct {
	...
	// global in-memory apiservices
	globalCommonAPIServices []*apiregistration.APIService
}
...
func (c completedConfig) NewWithDelegate(delegationTarget genericapiserver.DelegationTarget) (*APIAggregator, error) {
	...
	apisHandler := &apisHandler{
		...
		lister: s.lister,
		commonAPIServicesGetter: func(tenant multitenancy.TenantInfo) []*apiregistration.APIService {
			commonAPIServicesWithTenantInfo := make([]*apiregistration.APIService, len(s.globalCommonAPIServices))
			for i := range s.globalCommonAPIServices {
				commonAPIServicesWithTenantInfo[i] = s.globalCommonAPIServices[i].DeepCopy()
				commonAPIServicesWithTenantInfo[i].Annotations = util.TransformTenantInfoToAnnotations(tenant)
			}
			return commonAPIServicesWithTenantInfo
		},
	}
	...
	if feature.DefaultFeatureGate.Enabled(multitenancy.FeatureName) {
		// disable openapi controller since that we might not want this in a short-term :)
		return s, nil
	}
...
func (s *APIAggregator) AddAPIService(apiService *apiregistration.APIService) error {
	...
	proxyHandler := &proxyHandler{
		...
		handlingInfo:    make(map[util.TenantHash]*atomic.Value),
	}
	proxyHandler.updateAPIService(apiService)
	if s.openAPIAggregationController != nil {
