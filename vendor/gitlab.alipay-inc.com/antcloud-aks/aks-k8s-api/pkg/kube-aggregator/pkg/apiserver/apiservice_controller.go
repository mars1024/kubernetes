...
func (c *APIServiceRegistrationController) ShallowCopyWithTenant(tenant multitenancy.TenantInfo) interface{} {
	copied := *c
...
func (c *APIServiceRegistrationController) sync(key string) error {
	...
	c = c.ShallowCopyWithTenant(tenant).(*APIServiceRegistrationController)
	key = name
