...
type apisHandler struct {
	...
	commonAPIServicesGetter CommonAPIServicesGetter
}
...
func (r *apisHandler) ShallowCopyWithTenant(tenant multitenancy.TenantInfo) interface{} {
	copied := *r
	copied.lister = r.lister.(multitenancymeta.TenantWise).ShallowCopyWithTenant(tenant).(listers.APIServiceLister)
	return &copied
}
...
func (r *apisHandler) ServeHTTP(w http.ResponseWriter, req *http.Request) {
	...
	if err != nil {
		http.Error(w, "missing tenant info in user", http.StatusInternalServerError)
		return
	}

	r = r.ShallowCopyWithTenant(tenant).(*apisHandler)
	...
	apiServices = mergeDuplicatedAPIServices(apiServices, r.commonAPIServicesGetter(tenant))
...
func (r *apiGroupHandler) ShallowCopyWithTenant(tenant multitenancy.TenantInfo) interface{} {
	copied := *r
	copied.lister = r.lister.(multitenancymeta.TenantWise).ShallowCopyWithTenant(tenant).(listers.APIServiceLister)
	return &copied
}
...
func (r *apiGroupHandler) ServeHTTP(w http.ResponseWriter, req *http.Request) {
	...
	r = r.ShallowCopyWithTenant(tenant).(*apiGroupHandler)
