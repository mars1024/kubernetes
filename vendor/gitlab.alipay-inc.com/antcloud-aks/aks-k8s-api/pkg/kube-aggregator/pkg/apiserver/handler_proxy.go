...
type proxyHandler struct {
	...
	handlingInfo map[multitenancyutil.TenantHash]*atomic.Value
}
...
func (r *proxyHandler) ServeHTTP(w http.ResponseWriter, req *http.Request) {
	...
	if err != nil {
		http.Error(w, "no tenant found in request context", http.StatusBadRequest)
	}

	if _, ok := r.handlingInfo[multitenancyutil.GetHashFromTenant(tenant)]; !ok {
		r.localDelegate.ServeHTTP(w, req)
		return
	}

	value, ok := r.handlingInfo[multitenancyutil.GetHashFromTenant(tenant)]
	if !ok || value.Load() == nil {
		r.localDelegate.ServeHTTP(w, req)
		return
	}
	handlingInfo := value.Load().(proxyHandlingInfo)
...
func (r *proxyHandler) updateAPIService(apiService *apiregistrationapi.APIService) {
	tenant, err := multitenancyutil.TransformTenantInfoFromAnnotations(apiService.Annotations)
	if err != nil {
		glog.Warningf("apiservice %s missing tenant annotation", apiService.Name)
		return
	}

	if apiService.Spec.Service == nil {
		tenantHash := multitenancyutil.GetHashFromTenant(tenant)
		if r.handlingInfo[tenantHash] == nil {
			r.handlingInfo[tenantHash] = &atomic.Value{}
		}
		r.handlingInfo[multitenancyutil.GetHashFromTenant(tenant)].Store(proxyHandlingInfo{local: true})
		return
	}
