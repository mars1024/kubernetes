...
import (
	...
	"gitlab.alipay-inc.com/antcloud-aks/aks-k8s-api/pkg/kube-aggregator/pkg/controllers"
)
...
type autoRegisterController struct {
	...
	commonAPIHandlerManager CommonAPIHandlerManager
}

func NewAutoRegisterController(apiServiceInformer informers.APIServiceInformer, apiServiceClient apiregistrationclient.APIServicesGetter, manager CommonAPIHandlerManager) *autoRegisterController {
	c := &autoRegisterController{
		...
		commonAPIHandlerManager: manager,
	}
	...
	apiServiceInformer.Informer().AddEventHandler(cache.ResourceEventHandlerFuncs{
		AddFunc: func(obj interface{}) {
			...
			key, err := multitenancycache.MultiTenancyKeyFuncWrapper(cache.DeletionHandlingMetaNamespaceKeyFunc)(cast)
			if err != nil {
				utilruntime.HandleError(err)
				return
			}
			if isAutomanagedOnStart(cast) {
				return
			}
			c.queue.Add(key)
		},
...
func (c *autoRegisterController) ShallowCopyWithTenant(tenant multitenancy.TenantInfo) interface{} {
	copied := *c
	copied.apiServiceLister = c.apiServiceLister.(multitenancymeta.TenantWise).ShallowCopyWithTenant(tenant).(listers.APIServiceLister)
	copied.apiServiceClient = c.apiServiceClient.(multitenancymeta.TenantWise).ShallowCopyWithTenant(tenant).(apiregistrationclient.APIServicesGetter)
	return &copied
}
...
func (c *autoRegisterController) checkAPIService(name string) (err error) {
	...
	tenant, _, name, err := multitenancycache.MultiTenancySplitKeyWrapper(cache.SplitMetaNamespaceKey)(name)
	if err != nil {
		return err
	}
	c = c.ShallowCopyWithTenant(tenant).(*autoRegisterController)
...
func (c *autoRegisterController) AddAPIServiceToSyncOnStart(in *apiregistration.APIService) {
	c.commonAPIHandlerManager.AddCommonAPIService(in)
}
...
func (c *autoRegisterController) addAPIServiceToSync(in *apiregistration.APIService, syncType string) {
	...
	key, err := multitenancycache.MultiTenancyKeyFuncWrapper(cache.DeletionHandlingMetaNamespaceKeyFunc)(apiService)
	if err != nil {
		utilruntime.HandleError(fmt.Errorf("apiservice %v missing tenant annotation", apiService.Name))
		return
	}
	c.apiServicesToSync[key] = apiService
	c.queue.Add(key)
}

func (c *autoRegisterController) RemoveAPIServiceToSync(key string) {
	c.apiServicesToSyncLock.Lock()
	defer c.apiServicesToSyncLock.Unlock()

	delete(c.apiServicesToSync, key)
	c.queue.Add(key)
}

func (c *autoRegisterController) hasSyncedSuccessfully(key string) bool {
	c.syncedSuccessfullyLock.RLock()
	defer c.syncedSuccessfullyLock.RUnlock()
	return c.syncedSuccessfully[key]
}

func (c *autoRegisterController) setSyncedSuccessfully(key string) {
	c.syncedSuccessfullyLock.Lock()
	defer c.syncedSuccessfullyLock.Unlock()
	c.syncedSuccessfully[key] = true
}
