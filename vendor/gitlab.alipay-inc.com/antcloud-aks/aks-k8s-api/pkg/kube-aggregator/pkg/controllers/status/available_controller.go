...
func (c *AvailableConditionController) ShallowCopyWithTenant(tenant multitenancy.TenantInfo) interface{} {
	copied := *c
	...
	copied.endpointsLister = c.endpointsLister.(multitenancymeta.TenantWise).ShallowCopyWithTenant(tenant).(v1listers.EndpointsLister)
	copied.serviceLister = c.serviceLister.(multitenancymeta.TenantWise).ShallowCopyWithTenant(tenant).(v1listers.ServiceLister)
	...
	copied.serviceResolver = c.serviceResolver.(multitenancymeta.TenantWise).ShallowCopyWithTenant(tenant).(ServiceResolver)
	return &copied
}
...
func (c *AvailableConditionController) sync(key string) error {
	...
	// NOTE(zuoxiu.jm): bypass admin apiservice for a backward-compatibility to suppress flooding errors
	if reflect.DeepEqual(tenant, multitenancy.GlobalAdminTenant) {
		return nil
	}
	c = c.ShallowCopyWithTenant(tenant).(*AvailableConditionController)
	key = name
...
func (c *AvailableConditionController) enqueue(obj *apiregistration.APIService) {
	if isAutomanagedOnStart(obj) {
		return
	}

	key, err := multitenancycache.MultiTenancyKeyFuncWrapper(cache.DeletionHandlingMetaNamespaceKeyFunc)(obj)
	if err != nil {
...
func isAutomanagedOnStart(service *apiregistration.APIService) bool {
	...
	return service.Annotations["kube-aggregator.kubernetes.io/automanaged"] == "onstart"
}
