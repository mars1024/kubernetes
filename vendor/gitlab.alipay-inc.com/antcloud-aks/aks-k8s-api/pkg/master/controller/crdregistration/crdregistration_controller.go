...
type crdRegistrationController struct {
	...
	syncHandler func(tgv util.TenantGroupVersion) error
	...
	if crds, err := c.crdLister.List(labels.Everything()); err != nil {
		...
		for _, crd := range crds {
			...
			for _, version := range crd.Spec.Versions {
				if err := c.syncHandler(util.TenantGroupVersion{
...
func (c *crdRegistrationController) processNextWorkItem() bool {
	...
	err := c.syncHandler(key.(util.TenantGroupVersion))
	if err == nil {
...
func (c *crdRegistrationController) enqueueCRD(crd *apiextensions.CustomResourceDefinition) {
	...
	for _, version := range crd.Spec.Versions {
		c.queue.Add(util.TenantGroupVersion{
			TenantHash:   multitenancyutil.GetHashFromTenant(tenant),
			GroupVersion: schema.GroupVersion{Group: crd.Spec.Group, Version: version.Name},
		})
	}
...
func (c *crdRegistrationController) handleVersionUpdate(tgv util.TenantGroupVersion) error {
	groupVersion := tgv.GroupVersion
		...
		for _, version := range crd.Spec.Versions {
			...
			c.apiServiceRegistration.AddAPIServiceToSync(&apiregistration.APIService{
				ObjectMeta: metav1.ObjectMeta{
					Name:        apiServiceName,
					Annotations: multitenancyutil.TransformTenantInfoToAnnotations(tgv.TenantHash.TenantInfo()),
				},
	...
	c.apiServiceRegistration.RemoveAPIServiceToSync(multitenancyutil.TransformTenantInfoToJointString(tgv.TenantHash.TenantInfo(), "/") + "/" + apiServiceName)
	return nil
}
