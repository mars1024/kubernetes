...
func WithImpersonation(handler http.Handler, a authorizer.Authorizer, s runtime.NegotiatedSerializer) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, req *http.Request) {
		...
		requestingAdminTenant, _ := multitenancyutil.TransformTenantInfoFromUser(requestor)
		...
		for _, impersonationRequest := range impersonationRequests {
			...
			// NOTE(zuoxiu.jm): dirty hack here, we bypass impersonation for admin user
			if (multitenancyutil.IsMultiTenancyWiseAdmin(requestor.GetName()) && reflect.DeepEqual(requestingAdminTenant, multitenancy.GlobalAdminTenant)) || multitenancyutil.IsMultiTenancyWiseTenant(requestingAdminTenant) {
				...
				reason := fmt.Sprintf("impersonation header is forbidden: requestor(%v)", requestor)
		...
		targetAdminTenant, _ := multitenancyutil.TransformTenantInfoFromUser(newUser)
		if strings.HasPrefix(targetAdminTenant.GetTenantID(), multitenancy.GlobalAdminTenantNamePrefix) {
			if reflect.DeepEqual(requestingAdminTenant, multitenancy.GlobalAdminTenant) {
				// allow impersonation for legacy global admin
			} else if !reflect.DeepEqual(requestingAdminTenant, targetAdminTenant) {
				reason := fmt.Sprintf("across admin-tenants access is forbidden: %v -> %v", requestingAdminTenant, targetAdminTenant)
				glog.V(4).Infof("Forbidden: %#v, Reason: %s, Error: %v", req.RequestURI, reason, err)
